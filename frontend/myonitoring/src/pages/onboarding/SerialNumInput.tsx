...
const SerialNumberInput = () => {
  ...
  const handleNext = async () => {
    ...
    //   // 성공 시 다음 단계로 이동
      navigate("/connection-success"); // 실제 경로로 변경
    } catch (error) {
    ...
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <ExceptTopContentSection>
        <div>
          ...
          어플 연동을 위해 기기 시리얼 넘버가 필요합니다. 기기 하단의 시리얼 넘버를 입력해 주세요.
          </p>
          ...
          <div className="mt-6">
            ...
            />
            {error && (
              <p className="text-red-500 text-xs mt-1">시리얼 넘버를 입력해주세요.</p>
            )}
          </div>
      ...
      <footer className="fixed bottom-0 left-0 w-full p-4 bg-white">
        <WideButton
          ...
          bgColor={serialNumber.trim() ? "bg-darkGray" : "bg-gray-300 cursor-not-allowed"}
          onClick={handleNext}
        />
      </footer>
    </div>
