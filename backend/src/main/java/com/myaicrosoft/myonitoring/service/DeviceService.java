...
public class DeviceService {
    ...
    public Device createDevice(DeviceCreateRequest request, Long userId) {
        User2 user2 = userRepository2.findById(userId)
    ...
    public DeviceDetailResponseDto getDeviceById(Long deviceId) {
        ...
        // 고양이 정보 추출 (기기에 연결된 고양이가 없을 수도 있음)
        ...
        // 유저 정보 추출
        List<DeviceDetailResponseDto.UserInfo> users = device.getUser2s().stream()
                .map(user -> new DeviceDetailResponseDto.UserInfo(user.getId(), user.getUsername()))
                .collect(Collectors.toList());
    ...
    public void deleteDeviceById(Long deviceId) {
        if (!deviceRepository.existsById(deviceId)) {
            throw new IllegalArgumentException("해당 ID의 기기를 찾을 수 없습니다. ID: " + deviceId);
        }
